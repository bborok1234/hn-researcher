...
ENABLE_COMPANY="${ENABLE_COMPANY:-0}"; OPEN_BROWSER="${OPEN_BROWSER:-1}"
...
NEW=$(python3 build_profile.py --check PROFILE.md COMPANY.md 2>/dev/null || true)
if [ "$(date +%u)" = "1" ] || [ -n "$NEW" ]; then
  [ -n "$NEW" ] && echo "새 프로젝트 감지: $(echo $NEW | tr '\n' ' ')" >> "logs/$DATE.log"
  ./profile.sh >> "logs/$DATE.log" 2>&1
  [ "$ENABLE_COMPANY" = "1" ] && ./company.sh >> "logs/$DATE.log" 2>&1
fi
...
# 월요일: 회사 사업화 심층 리포트 (개인용 리포트가 완성된 뒤 실행 — 중복 제안 방지에 참고한다)
# if로 쓴다 — `[ ... ] && cmd`를 마지막 줄에 두면 월요일이 아닌 날 스크립트가 실패(1)로 끝난다
if [ "$(date +%u)" = "1" ] && [ "$ENABLE_COMPANY" = "1" ]; then
